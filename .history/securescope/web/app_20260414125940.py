            ...
            findings.append(
                {
                    ...
                    "frameworks": check.get("frameworks") or _frameworks_for_check(check.get("check")),
...
def _findings_mapped_to_control(framework: str, control_id: str, org_id: str | None) -> list[dict]:
    out: list[dict] = []
    seen: set[str] = set()
...
def _compliance_payload(framework: str, org_id: str | None) -> dict:
    ...
    for control_id, control_info in controls_def.items():
        ...
        control_status[control_id] = {
            ...
            "remediated_count": sum(1 for f in mapped_findings if (f.get("status") or "").lower() in ("closed", "remediated")),
...
def _read_check_workflow_status(finding_id: str) -> str | None:
    ...
    if idx < 0 or idx >= len(checks):
        return None
    return (checks[idx].get("workflow_status") or checks[idx].get("status") or "new") or "new"
...
def has_permission(_org_id, perm: str) -> bool:
    ...
    PERMISSIONS = {
        'super_admin': [
            ...
            'license_view', 'license_create', 'license_update', 'license_delete',
            'org_view', 'org_create', 'org_update',
            'finding_view', 'finding_update', 'finding_close',
            'target_view', 'target_create', 'target_scan', 'target_delete',
            'report_create', 'report_view', 'report_export',
            'compliance_view',
            'settings_view', 'settings_edit'
        ],
        'org_admin': [
            'user_create', 'user_view', 'user_update',  # org-scoped
            'finding_view', 'finding_update', 'finding_close',
            'target_view', 'target_create', 'target_scan',
            'report_create', 'report_view', 'report_export',
            'compliance_view',
            'settings_view', 'settings_edit'  # org-scoped
        ],
        'security_manager': [
            'finding_view', 'finding_update', 'finding_close',
            'target_view', 'target_create', 'target_scan',
            'report_create', 'report_view', 'report_export',
            'compliance_view',
            'remediation_view', 'remediation_update'
        ],
        'normal_user': [
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if "last_seen" in session:
            try:
                if now_ts - float(session["last_seen"]) > app.permanent_session_lifetime.total_seconds():
                    session.clear()
                    return _login_or_api_unauthorized()
...
def dashboard_kpis():
    ...
    for host, points in scan_history.items():
        for p in points[-3:]:
            trend.append({"target": host, "time": p.get("time"), "score": int(p.get("score", 0))})
    ...
    return jsonify(
        {
            ...
            "score_trend": trend,
            "llm_health": "healthy" if llm_store.list_models(_llm_user_id()) else "not_configured",
        }
    )
...
def api_targets():
    sort_by = (request.args.get("sort") or request.args.get("sort_by") or "name").lower()
    order = (request.args.get("order") or request.args.get("sort_order") or "asc").lower()
    ...
    if sort_by == "score":
        ...
        all_targets.sort(key=lambda x: _parse_last_scan_ts(x["last_scan"]) or datetime.min, reverse=reverse)
    elif sort_by == "status":
...
def api_scan_target(target_id):
    ...
    if target_key not in ("localhost",) and not target_key.startswith("web-") and not target_key.startswith("ports-"):
        return jsonify(
            {
    ...
    worker.start()
    return jsonify({"message": "Scan queued", "target_id": target_id, "status": "scanning"})
...
def list_findings():
    ...
    if cfw and cctrl and cfw in COMPLIANCE_FRAMEWORKS:
        allowed_keys = [k for k, fm in FINDING_TO_CONTROL_MAP.items() if cctrl in fm.get(cfw, [])]
        if allowed_keys:
            allow = set(allowed_keys)
            items = [f for f in items if _finding_compliance_keys(f) & allow]
    if target:
        items = [f for f in items if target in (f["target"] or "").lower() or target in (f.get("target_key") or "").lower()]
    if q:
        items = [
            f
            for f in items
...
def bulk_findings_action():
    ...
    for fid in finding_ids:
        if action == "mark_reviewed":
            ...
            ok = _apply_finding_update(fid, {"assigned_to": payload.get("assigned_to"), "status": "assigned"})
        elif action == "close":
        ...
        if ok:
            updated += 1
    return jsonify({"ok": True, "updated": updated, "requested": len(finding_ids), "action": action})
...
def api_update_remediation(finding_id):
    ...
    if "status" in data and data["status"] in ("not_started", "in_progress", "verification", "closed"):
        ...
        _apply_finding_update(finding_id, {"status": wf_map.get(data["status"], "in_progress")})
...
def generate_compliance_report(framework: str, target_ids: list, org_id: str | None) -> dict:
    if framework not in COMPLIANCE_FRAMEWORKS:
...
def generate_executive_report(framework: str | None, target_ids: list, org_id: str | None) -> dict:
    findings_part = generate_findings_report(target_ids, org_id)
    comp_part = _compliance_payload(framework, org_id) if framework and framework in COMPLIANCE_FRAMEWORKS else None
    return {
...
def _attach_report_branding(report_data: dict, org_id: str | None) -> dict:
    ...
    brand_org = (config.get("branding", {}) or {}).get("organization_name", "NiTechSpark")
    display = brand_org
    if org_id:
...
def render_report_html(report_data: dict, report_type: str) -> str:
    ...
    org_line = html_escape(str(report_data.get("organization_display_name") or report_data.get("organization") or ""))
    if org_line in ("None", ""):
        org_line = html_escape(str((config.get("branding", {}) or {}).get("organization_name", "NiTechSpark")))
...
"""
        for _cid, control in (report_data.get("controls") or {}).items():
            ...
            if n > 0 and findings:
                ...
                detail_block = f"<span>{n} mapped (open full JSON export for IDs)</span>"
            else:
                detail_block = "—"
            html += f"""    <tr>
...
def generate_pdf_report(report_data: dict, report_type: str) -> bytes:
    ...
    story = [
        ...
        Spacer(1, 6),
        Paragraph(f"Generated: {report_data.get('generated_at', '')}", styles["Normal"]),
        Spacer(1, 12),
    ]
    if report_type == "findings":
        ...
        story.append(
            Paragraph(f"Compliance: {report_data.get('compliance_percentage', 0)}%", styles["Heading2"])
        )
        rows = [["Control", "Title", "Status"]]
        for _cid, c in (report_data.get("controls") or {}).items():
            rows.append([str(c.get("control_id", "")), str(c.get("title", ""))[:50], str(c.get("status", ""))])
        story.append(Table(rows, repeatRows=1))
    else:
        story.append(Paragraph("Executive summary — see HTML or JSON export for details.", styles["Normal"]))
    doc.build(story)
    return buf.getvalue()
...
def api_generate_report():
    ...
    if report_type == "compliance":
        ...
        report_data = generate_executive_report(framework if framework in COMPLIANCE_FRAMEWORKS else None, target_ids, org_id)
    else:
...
@app.route('/api/llm/models/<model_id>', methods=['GET'])
@login_required
...
@app.route('/api/llm/models/<model_id>', methods=['PUT'])
@login_required
@secure_post
def update_llm_model(model_id):
    try:
        data = request.json or {}
        success = llm_store.update_model(
            model_id=model_id,
            user_id=_llm_user_id(),
        ...
        if not success:
            return jsonify({"error": "Failed to update model or not found"}), 404
...
def test_llm_connection():
    try:
        ...
        if not endpoint.startswith("http"):
            return jsonify({"status": "success", "message": "Connection assumes local mapping."})
        ...
        return jsonify({"status": "success", "message": "Connection established successfully."})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def create_admin_user():
    ...
    if session.get('role') == 'org_admin' and payload.get('org_id') != session.get('org_id'):
