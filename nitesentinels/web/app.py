...
from nitesentinels.core.utils import detect_platform
from nitesentinels.scanners.llm_scanner import LLMSecurityScanner
from nitesentinels.web.llm_store import LLMStore
from nitesentinels.web.report_generator import LLMReportGenerator
from nitesentinels.integrations.slack import send_slack_alert
from nitesentinels.integrations.email import send_email_report
from nitesentinels.core.compliance.dpdp_2023 import DPDP_2023_FRAMEWORK
...
scheduler.start()
db_path = os.environ.get("NITESENTINELS_DB_PATH")
if not db_path:
...
# HAM module — database store
from nitesentinels.web.ham_store import HAMStore
ham_store = HAMStore(db_path)
ham_store.init_db()
from nitesentinels.web.ham_demo_fixtures import seed_ham_demo_data
try:
    seed_ham_demo_data(ham_store, org_id="demo-org-1")
except Exception:
    pass

from nitesentinels.web.demo_routes import demo_bp
from nitesentinels.web.port_scan_routes import port_scan_bp
from nitesentinels.web.ham_routes import ham_bp
from nitesentinels.web.engagement_routes import engagement_bp
app.register_blueprint(demo_bp)
app.register_blueprint(port_scan_bp)
app.register_blueprint(ham_bp)
...
SUPER_ADMINS = set(
    (
        os.environ.get(
            "NITESENTINELS_SUPER_ADMINS", "nitechspark,nitechspark_admin"
        ).split(",")
    )
)
FINDING_STATUS_FLOW = (
...
def has_permission(_org_id, perm: str) -> bool:
    ...
    PERMISSIONS = {
        "super_admin": [
            ...
            "ham.reports.view",
            "ham.reports.export",
            # Assessment lifecycle — super_admin has all
            "client.view", "client.create", "client.edit", "client.delete",
            "engagement.view", "engagement.create", "engagement.edit", "engagement.manage",
            "finding.view", "finding.create", "finding.edit", "finding.close",
            "evidence.view", "evidence.add",
            "dlp.view", "dlp.manage",
            "retest.view", "retest.conduct",
        ...
        "org_admin": [
            ...
            "ham.reports.view",
            "ham.reports.export",
            # Assessment lifecycle — org_admin
        ...
        "security_manager": [
            ...
            "ham.assets.assign",
            "ham.reports.view",
            # Assessment lifecycle — security_manager
            "client.view",
        ...
        "normal_user": [
            ...
            "ham.assets.view",
            # Assessment lifecycle — normal_user (read-only)
            "client.view",
            "engagement.view",
            "finding.view",
            "evidence.view",
        ...
        "viewer": [
            ...
            "ham.assets.view",
            "ham.reports.view",
            # Assessment lifecycle — viewer (read-only)
            "client.view",
            "engagement.view",
            "finding.view",
            "evidence.view",
...
def create_app(config_overrides: dict | None = None) -> Flask:
    """Factory for tests and external usage — returns the Flask `app` instance.

    If `config_overrides` is provided, it will update the app config (useful for testing).
    """
    if config_overrides:
...
@login_required
def platform_hub():
    """NITECHSPARK Platform Hub — Central post-login module selection screen."""
...
def security_dashboard():
    """Security Audit Dashboard — Host auditing, posture checks, findings, and hardening."""
    plat_info = detect_platform()
...
# Alias for backward compatibility with existing tests and url_for('dashboard')
dashboard = security_dashboard
app.view_functions["dashboard"] = security_dashboard


@app.route("/compliance", methods=["GET"])
@login_required
def compliance_module():
    """NITECHSPARK Compliance Module — Standards mapping and baseline guidance."""
    return render_template(
        "compliance.html",
        username=session.get("username", "operator"),
        role=session.get("role", "viewer"),
...
def reports_workspace():
    """NITECHSPARK Reports & AI Intelligence Workspace."""
    return render_template(
        "reports_workspace.html",
        username=session.get("username", "operator"),
        role=session.get("role", "viewer"),
        is_super_admin=_is_super_admin(),
        org_name=config.get("branding", {}).get("organization_name", "NiTechSpark"),
...
@login_required
def admin_workspace():
    """NITECHSPARK Platform Administration Workspace."""
    return render_template(
...
def system_workspace():
    """NITECHSPARK System & Configuration Workspace."""
    return render_template(
        "system_workspace.html",
        username=session.get("username", "operator"),
        role=session.get("role", "viewer"),
        is_super_admin=_is_super_admin(),
        org_name=config.get("branding", {}).get("organization_name", "NiTechSpark"),
