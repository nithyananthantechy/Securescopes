...
class SecretCipher:
    ...
    def __init__(self) -> None:
        key_raw = os.environ.get("NITESENTINELS_LLM_KEY", "").encode("utf-8")
        if not key_raw:
            # Stable process-local fallback derived from SECRET_KEY.
            seed = os.environ.get("SECRET_KEY", "nitesentinels-llm-default").encode("utf-8")
...
class LLMStore:
    ...
    def add_schedule(self, user_id: str, target: str, ports: str, frequency: str, next_run: str, notify_email: int) -> bool:
        with self._conn() as conn:
            conn.execute(
                "INSERT INTO scan_schedules (user_id, target, ports, frequency, next_run, notify_email, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (user_id, target, ports, frequency, next_run, notify_email, _utc_now())
            )
        return True

    def delete_schedule(self, sid: int, user_id: str) -> bool:
        with self._conn() as conn:
            cur = conn.execute(
